...
| Four-model generation order | compliant in code/tests | `FailoverGenerationClient` builds the ordered Cloudflare/NVIDIA/Cloudflare attempts (`apps/api/Generation/FailoverGenerationClient.cs:14-22`), and the locked-order test covers the current models (`tests/api/Generation/GenerationClientTests.cs:480-511`). Phase 6 only live-exercised the historical NVIDIA path (`docs/PHASE6_RUNBOOK.md:38-41`). |
