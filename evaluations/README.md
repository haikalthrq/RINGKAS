...
All `RAGAS_LLM_*` numeric values must be positive. Recommended configuration for `@cf/openai/gpt-oss-120b`: timeout 300 seconds (allowing full reasoning completion), retries 2, one worker, preflight 20, output cap 32,000, and temperature 0.1. Each sample/metric is executed in an isolated child process (`ragas_metric_worker.py`) to prevent asyncio loop collisions and ensure clean per-metric timeout enforcement. Preflight scores its configured samples progressively and stops the baseline if any required metric is missing or non-finite.

The harness atomically checkpoints every finite metric beside `responses.json`. Preflight has completed successfully with `preflight_validated` (20 samples, 60 metrics; Faithfulness: 0.4083, Context Precision: 0.0375, Context Recall: 0.1250). A resume into the full 100-sample baseline is allowed only when the model, provider, output cap, timeout, retry count, worker count, RAGAS version, temperature, and selected sample IDs exactly match. `completed` is emitted only when all 100 samples have finite `faithfulness`, `context_precision`, and `context_recall` values. The 1000-response automated audit remains separate from this 100-sample RAGAS baseline.

Lihat `metrics_summary.md` untuk hasil otomatis dan preflight RAGAS.
